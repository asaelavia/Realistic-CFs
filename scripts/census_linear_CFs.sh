#!/bin/bash
# Census Dataset - Linear Model CFS With Integrated Model
echo "Census linear model CFs experiment starts!"
echo "Check logs/census_linear_CFS.out for detailed output"
python -u perturb_test.py \
    ...
    --num_samples 11 \
    --epochs 2 \
    --exp_name census_linear_CFS \
    --solver_timeout 300000 \
    --timeout 200000 \
    --linear_model \
    > logs/census_linear_CFS.out

echo "Census linear model CFs experiment complete!"
echo "Results saved to: data/census_linear_CFS/"
echo "Check logs/census_linear_CFS.out for detailed output"